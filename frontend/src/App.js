...
import { isValidToken } from "./helpers/AuthHelper";
...
function MainApp() {
  ...
  if (
    window.localStorage.getItem("appJornalesToken") !== null &&
    isValidToken()
  ) {
    ...
    return (
      <div className="App justify-content-center">
        {personaRolLoggeado?.personaRol?.rol?.rol === "ADMINISTRADOR" &&
          !isChangePasswordPage && <HeaderAdminComponent />}
        {personaRolLoggeado?.personaRol?.rol?.rol === "JEFE_OBRA" &&
          !isChangePasswordPage && <HeaderJefeComponent />}
        {personaRolLoggeado?.personaRol?.rol?.rol === "TRABAJADOR" &&
          !isChangePasswordPage && <HeaderTrabajadorComponent />}
        {!personaRolLoggeado && (
          <Routes>
