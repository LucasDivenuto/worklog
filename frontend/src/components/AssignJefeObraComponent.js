...
export const AssignJefeObraComponent = () => {
  ...
  useEffect(() => {
    const fetchEsJefe = async () => {
      if (persona) {
        try {
          const esJefeData = await PersonaRolService.getEsJefeObraByPersonaId(
            persona.id
          );
  ...
  const asignarJefeObra = async (e) => {
    if (isJefeObra) {
      ...
      JefeObraService.createJefeObra(nuevoJefeObra)
        .then((res) => {
          navigate("/obras");
        })
        .catch((e) => {
    ...
    <div className="mt-5 row justify-content-center col-12">
      ...
      {!persona && (
        <div className="row justify-content-center">
          <ContainerPersonaFinderComponent
            minimalData={true}
            onCancelar={volver}
      ...
      {persona && (
        <div className="col-12 col-lg-7 row justify-content-center">
          ...
          {isJefeObra === true && (
            <>
              <button
                className="btn btn-success py-2 m-3 col-4"
                type="submit"
                onClick={(e) => asignarJefeObra(e)}
              >
                Asignar
              </button>
              <button
                className="btn btn-danger py-2 m-3 col-4"
                type="submit"
                onClick={cancelar}
              >
                Cancelar
      ...
      {!isJefeObra && persona && (
        <div className="col-12 col-lg-7 row justify-content-center">
          <div className="alert alert-danger" role="alert">
            La persona encontrada no tiene el rol adecuado para ser jefe de obra
          </div>
          <button
            className="btn btn-danger py-2 m-3 col-4"
            type="submit"
